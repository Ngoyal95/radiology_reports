...
from keras.layers import Input, Dense, Activation, Embedding, SpatialDropout1D, concatenate, Dropout
...
y_train = np.asarray([x for x in df_train['fu_label']]).astype(np.float32)
y_test = np.asarray([x for x in df_test['fu_label']]).astype(np.float32)
